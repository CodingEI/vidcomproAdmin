...
$(".cash-out-switcher .form-check .form-check-input").change(function () {
  if (this.checked) {
    $(this)
      ...
      .parent()
      .parent()
      .find(".cashout-spinner-wrapper input")
      .attr("disabled", false);
    $(this)
      .parent()
      .parent()
...
function bet_amount_incremental(element) {
  var bet_amount = parseFloat(
    $(element).parent().parent().find(".input #bet_amount").val(),
  );
  bet_amount++;
...
function select_direct_bet_amount(element) {
  var current_bet_amount = parseFloat(
    $(element).parent().parent().find(".input #bet_amount").val(),
  );
  var adding_bet_amount = parseFloat($(element).find(".amt").text()).toFixed(2);
  if ($(element).hasClass("same")) {
    var new_bet_amount = parseFloat(
      parseFloat(current_bet_amount) + parseFloat(adding_bet_amount),
    ).toFixed(2);
    if (new_bet_amount <= max_bet_amount) {
      $(element)
        .parent()
        .parent()
        .find(".input #bet_amount")
        .val(new_bet_amount);
    }
  } else {
...
function cash_out_now_old(element, section_no, increment = "") {
  if (section_no == 0) {
    cashOutSound();
  } else {
  ...
  console.log({ incrementor, bet_amount });
  // let incrementor = $("#auto_increment_number").text().slice(0,-1);
  game_id = current_game_data.id;

  // if (currency_id == 1) {
  var amt = parseFloat(
    parseFloat(incrementor) * parseFloat(bet_amount),
  ).toFixed(2);
  // } else {
  // var amt = parseFloat(
  //   parseFloat(incrementor) * (parseFloat(bet_amount) / 80),
  // ).toFixed(2);
  // }

  $(
    "#all_bets .mCSB_container .bet_id_" + member_id + section_no + "",
  ).addClass("active");
  $(
    "#all_bets .mCSB_container .bet_id_" +
      member_id +
      section_no +
      " .column-3",
  ).html(
    '<div class="' +
      get_multiplier_badge_class(incrementor) +
    ...
    success: function (result) {
      if (result.isSuccess) {
        if (
          ...
          result.data.wallet_balance != NaN &&
          result.data.wallet_balance != "NaN"
        ) {
          $("#wallet_balance").text(
            currency_symbol + parseFloat(result.data.wallet_balance),
          );
          $("#header_wallet_balance").text(
            currency_symbol + parseFloat(result.data.wallet_balance),
          ); // Show Header Wallet Balance
        } else {
        ...
        if (section_no == 0) {
          ...
          $("#my_bet_list #my_bet_section_0 .column-3").html(
            '<div class="' +
              get_multiplier_badge_class(incrementor) +
              ' custom-badge mx-auto">' +
              incrementor +
          ...
          $("#my_bet_list #my_bet_section_0 .column-4").html(
            result.data.cash_out_amount + currency_symbol,
          );
          let is_main_auto_bet_checked = $("#main_auto_bet").prop("checked");
        ...
        if (section_no == 1) {
          ...
          $("#my_bet_list #my_bet_section_1 .column-3").html(
            '<div class="' +
              get_multiplier_badge_class(incrementor) +
              ' custom-badge mx-auto">' +
              incrementor +
          ...
          $("#my_bet_list #my_bet_section_1 .column-4").html(
            result.data.cash_out_amount + currency_symbol,
          );
          let is_extra_auto_bet_checked = $("#extra_auto_bet").prop("checked");
...
function new_game_generated() {
  ...
  if (is_main_auto_bet_checked) {
    if (
      bet_array.length != 2 &&
      (bet_array.length == 0 ||
        (bet_array.length == 1 && bet_array[0].section_no != 0))
    ) {
      var bet_type = $("#main_bet_now")
        .parent()
        .parent()
        .parent()
        .find(".navigation #bet_type")
        .val(); // 0 - Normal, 1 - Auto
      let bet_amount = $("#main_bet_now")
        .parent()
        .parent()
        .find(".bet-block .spinner #bet_amount")
        .val();
  ...
  if (is_extra_auto_bet_checked) {
    if (
      bet_array.length != 2 &&
      (bet_array.length == 0 ||
        (bet_array.length == 1 && bet_array[0].section_no != 1))
    ) {
      var bet_type = $("#extra_bet_now")
        .parent()
        .parent()
        .parent()
        .find(".navigation #bet_type")
        .val(); // 0 - Normal, 1 - Auto
      let bet_amount = $("#extra_bet_now")
        .parent()
        .parent()
        .find(".bet-block .spinner #bet_amount")
        .val();
...
function incrementor(inc_no) {
  ...
  document.getElementById("auto_increment_number").innerText =
    inc_no + "" + "x";
...
function cash_out_bet(cashOutData) {
  $("#all_bets .mCSB_container ." + cashOutData.hash_id + "").addClass(
    "active",
  );
  $("#all_bets .mCSB_container ." + cashOutData.hash_id + " .column-3").html(
    '<div class="' +
      get_multiplier_badge_class(cashOutData.incrementor) +
      ' custom-badge mx-auto">' +
      cashOutData.incrementor +
      "x</div>",
  );
  if (currency_id == 1) {
  ...
  $("#all_bets .mCSB_container ." + cashOutData.hash_id + " .column-4").html(
    amt + currency_symbol,
  );
}
...
function update_bet_list(bets, target, appendType = "") {
  ...
  for (i = 0; i < bets.length; i++) {
    ...
    if (parseFloat(bets[i].cashout_multiplier) > 0) {
      var cashOut =
        Math.round(bets[i].cashout_multiplier * bets[i].amount) +
        currency_symbol;
      var multiplication =
        '<div class="' +
        badgeColor +
        ' custom-badge mx-auto">' +
        bets[i].cashout_multiplier +
        "x</div>";
    } else {
      var cashOut = "-";
      var multiplication = "-";
    ...
    html +=
      ...
      " " +
      '">' +
      '<div class="column-1 users"> <img src="' +
      bets[i].image +
      '" class="avatar me-1"> ' +
      bets[i].userid +
      " </div>" +
      '<div class="column-2"> <button class="btn btn-transparent previous-history d-flex align-items-center mx-auto"> ' +
      bets[i].amount +
      currency_symbol +
      " </button> </div>" +
      '<div class="column-3"> ' +
...
function update_my_new_bet(bet_amount, section_no, target) {
  var html = "";
  html +=
    '<div class="list-items" id="my_bet_section_' +
    section_no +
    '">' +
    '<div class="column-1 users fw-normal"> ' +
    get_current_hour_minute() +
    " </div>" +
    '<div class="column-2"> <button class="btn btn-transparent previous-history d-flex align-items-center mx-auto fw-normal">' +
    parseFloat(bet_amount).toFixed(2) +
    "" +
    currency_symbol +
    "</button> </div>" +
  ...
  $.ajax({
    ...
    success: function (result) {
      if (result.isSuccess && Object.keys(result.data).length > 0) {
        ...
        $("#prev_win_multi")
          .addClass(get_multiplier_badge_class(winMulti))
          .text(parseFloat(winMulti).toFixed(2) + "x");
      } else {
...
function cash_out_multiplier(inc_no) {
  if (
    bet_array.length == 1 &&
    bet_array[0].section_no == 0 &&
    bet_array[0].is_bet != undefined
  ) {
    $("#main_bet_section")
      .find("#cash_out_amount")
      .text(
        parseFloat(bet_array[0].bet_amount * inc_no).toFixed(2) +
          "" +
          currency_symbol,
      );
  }
    ...
    $.map(bet_array, function (item, index) {
      if (item.section_no == 0 && item.is_bet != undefined) {
        $("#main_bet_section")
          .find("#cash_out_amount")
          .text(
            parseFloat(item.bet_amount * inc_no).toFixed(2) +
              "" +
              currency_symbol,
          );
      }
...
function place_bet_now() {
  ...
  $.ajax({
    // url: '/game/add_bet',
    url: "/api/webapi/aviator/add_bet",
    data: {
      //   _token: hash_id,
      all_bets: bet_array,
    },
    ...
    success: function (result) {
      if (result.isSuccess) {
        if (
          ...
          $("#wallet_balance").text(
            currency_symbol + result.data.wallet_balance,
          );
          $("#header_wallet_balance").text(
            currency_symbol + result.data.wallet_balance,
          ); // Show Header Wallet Balance
        ...
        if (bet_array.length == 1) {
          update_my_new_bet(
            bet_array[0].bet_amount,
            bet_array[0].section_no,
            "#my_bet_list .mCSB_container",
          );
          ...
          if (bet_array[1] != undefined) {
            update_my_new_bet(
              bet_array[1].bet_amount,
              bet_array[1].section_no,
              "#my_bet_list .mCSB_container",
            );
          }
        }
        ...
        $("#extra_bet_section .controls").removeClass("bet-border-red");
        $("#extra_bet_section .controls").removeClass("bet-border-yellow");
        $("#extra_bet_section .controls .navigation").removeClass(
          "stop-action",
        );
...
function update_round_history(inc_no) {
  var html =
    '<div class="' +
    get_multiplier_badge_class(inc_no) +
    ' custom-badge">' +
    parseFloat(inc_no).toFixed(2) +
    "x</div>";
  ...
  if (isChecked) {
    ...
    if (bet_amount >= min_bet_amount && bet_amount <= max_bet_amount) {
      if (bet_array.length == 1) {
        ...
        if (
          bet_array[0].section_no != section_no &&
          bet_array[1].section_no != section_no
        ) {
          bet_array.push({
            bet_type: bet_type,
            bet_amount: bet_amount,
...
function gameLoadingTimer() {
  ...
  var game_loading_timer = setInterval(function () {
    if (timer_no <= 5) {
      ...
      $(".waiting-text-2").text(`00:0${5 - timer_no}`);
      timer_no++;
    } else {
      $(".loading-game").removeClass("show");
...
function startLoadingTimer() {
  ...
  const timer = setInterval(() => {
    if (timeLeft === 0) {
      ...
      timeLeft--;
      $(".waiting-text2").text("00:0" + timeLeft);
    }
  }, 1000);
}
...
$(".fill-line").bind(
  "oanimationstart animationstart webkitAnimationStart",
  function () {
