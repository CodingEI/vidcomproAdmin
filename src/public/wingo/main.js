...
fetch("/api/webapi/GetUserInfo")
  ...
  .then((data) => {
    ...
    $("#bonus_balance_amount").text(
      `₹ ${formatIndianNumber(data.data.teamBusinessMoney)} `,
    );
  });
                  ...
                  <span data-v-373b3197="">${
                    ...
                    list_order.status === 1
                      ...
                      : '<span data-v-a9660e98="" class="fail"> - ₹ ' +
                        (
                          parseFloat(list_order.money) +
                          parseFloat(list_order.fee)
                        ).toFixed(2) +
                        "</span>"
                  }</span>
                  </div>`
...
function initGameLogics({
  ...
  $(".TimeLeft__C-name").html(gameNameMap[parseInt(GAME_TYPE_ID)]);
  $(".Betting__Popup-head-title").html(gameNameMap[parseInt(GAME_TYPE_ID)]);
