  ...
  try {
    ...
    if (user?.restricted == 1) {
      return res.status(200).json({
        message: "You are restricted from making deposits. Contact support",
        status: true,
      });
    }
...
const addManualUSDTPaymentRequest = async (req, res) => {
  try {
    ...
    let money = money_usdt * parseFloat(usdtExchangeRate);
    let utr = data.utr;
...
const getUserDataByAuthToken = async (authToken) => {
  let [users] = await connection.query(
    "SELECT `phone`, `code`,`name_user`,`invite`, `restricted` FROM users WHERE `token` = ? ",
    [authToken],
  );
  const user = users?.[0];
...
const rechargeTable = {
  ...
  create: async (newRecharge) => {
    ...
    await connection.query("UPDATE users SET is_active = 1 WHERE phone = ?", [
      newRecharge.phone,
    ]);
...
const withdrawableMoneyTransfer = async (req, res) => {
  try {
    let auth = req.cookies.auth;

    const [rows] = await connection.query(
      "SELECT phone, money, password, withdrawable_money FROM users WHERE token = ?",
      [auth],
    );
    ...
    const amount = parseFloat(user.withdrawable_money);
    ...
    await connection.query(
      "UPDATE users SET money = money + ? , withdrawable_money = withdrawable_money - ? WHERE phone = ?",
      [amount, amount, phone],
    );
