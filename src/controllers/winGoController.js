...
import { generateCommissionId, yesterdayTime } from "../helpers/games.js";
import {
  addRewards,
...
const distributeCommission = async () => {
  try {
    ...
    console.log("Error from distributeCommission: " + error);
    return { success: false, message: error.message };
  }
};
...
const handlingWinGo1P = async (typeid) => {
  try {
    ...
    for (let i = 0; i < order.length; i++) {
      ...
      const [users] = await connection.query(
        "SELECT `money`,`withdrawable_money` FROM `users` WHERE `phone` = ?",
        [phone],
      );

      let totals =
        parseFloat(users[0].withdrawable_money) + parseFloat(winAmount);

      // Validate if user has bet on both Big and small
      const [bigAndSmallBets] = await connection.query(
        "SELECT bet   FROM minutes_1 WHERE phone = ? AND game = ? AND stage = ? AND bet IN ('l','n') GROUP BY bet",
        [phone, game, winGoNow[0].period],
      );

      const isEligibleForWin = bigAndSmallBets.length < 2;
      ...
      if (isEligibleForWin) {
        ...
        await connection.query(
          "UPDATE `users` SET `withdrawable_money` = ? WHERE `phone` = ?",
          [totals, phone],
        );
      }
    }
...
const generateCommissionsForLost = async (period) => {
  ...
  while (true) {
    // Dynamically create placeholders for the validBets array

    const query = `
      SELECT 
        minutes_1.money AS money,
        minutes_1.phone AS phone,
        users.invite AS code
      FROM minutes_1
      LEFT JOIN users
        ON minutes_1.phone = users.phone
      WHERE 
        minutes_1.stage = ? 
        AND minutes_1.status = 2
        LIMIT ${batchSize} OFFSET ${offset}
    `;

    const [rows] = await connection.execute(query, [period]);
    ...
    rows.forEach(async (row) => {
      ...
      WITH RECURSIVE ParentCTE AS (
          ...
          -- Recursive member: Find the parent of the current user
          SELECT 
              u2.id AS user_id, 
          ...
          WHERE 
              p.level < 10 -- Stop recursion at level 10
      ),
      RankedCTE AS (
          -- Assign a row number for each phone to keep only the first occurrence
          SELECT 
              *, 
      ...
      -- Final query to fetch all unique results by phone
      SELECT 
          user_id, 
          name_user, 
          phone, 
    ...
    `,
      ...
      if (parentMembers.length > 0) {
        const comissAmount =
          (parseFloat(row.money) * 10) / 100 / parentMembers.length;

        for (const parentMember of parentMembers) {
          // await addRewards(
          //   parentMember.phone,
          //   comissAmount,
          //   "GAME LOSS REFERRAL BONUS",
          // );
          ...
          await updateUserWithdrawableMoney(parentMember.phone, comissAmount);
        }
      }
    });

    if (rows.length === 0) break; // Exit loop when no more rows are fetched
    offset += batchSize;

    // Add logic to process rows or perform updates here
  }
};
