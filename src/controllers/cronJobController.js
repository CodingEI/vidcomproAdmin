...
const cronJobGame1p = (io) => {
  ...
  // Runs every day at 2AM midnight
  cron.schedule("0 2 * * *", async () => {
    // await userController.distributeDailyInterest();
    await userController.distributeMonthlyLevelIncome();
    // await userController.distributeSalaries();
    // await userController.distributeExtraSalaryDaily();
    await gameController.autoCleanOldGames();
    ...
    await connection.execute("UPDATE users SET is_active = ?", [0]);
  });
