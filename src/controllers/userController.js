...
const aviator = async (req, res) => {
  let auth = req.cookies.auth;
  res.redirect(
    `https://api.bigmybull.com/theninja/src/api/userapi.php?action=loginandregisterbyauth&token=${auth}`,
  );
};
...
const userInfo = async (req, res) => {
  ...
  const [allSubordinates] = await connection.execute(
    `WITH RECURSIVE team AS (
         
          SELECT phone, id, code, invite, time, money, withdrawable_money
          FROM users
          WHERE code = ?
          
          UNION ALL
         
          SELECT u.phone, u.id, u.code, u.invite, u.time, u.money, u.withdrawable_money
          FROM users u
          INNER JOIN team t ON u.invite = t.code
      )
      SELECT SUM(money) as totalMainWalletMoney, SUM(withdrawable_money) as totalWithdrawableWalletMoney  FROM team WHERE invite != ? AND phone != ?`,
    [rows[0].code, rows[0].code, rows[0].phone],
  );
  return res.status(200).json({
    message: "Success",
    status: true,
    data: {
      ...
      withdrawable_amount: others.withdrawable_money,
      teamBusinessMoney: allSubordinates[0]?.totalMainWalletMoney || 0,
      teamBusinessWithdrawableMoney:
        allSubordinates[0]?.totalWithdrawableWalletMoney || 0,
    },
  ...
  try {
    ...
    return res.status(200).json({
      ...
      userInfo: {
        ...
        money_user: user[0].withdrawable_money,
...
const claimDailyAttendanceBonus = async (req, res, next) => {
  let auth = req.cookies.auth;
  const [user] = await connection.query(
    "SELECT `phone`,`money`, `code`,`invite` FROM users WHERE `token` = ? ",
    [auth],
  ...
  const [claimedReward] = await connection.execute(
    "SELECT * FROM claimed_rewards WHERE type = ? AND phone = ? AND DATE(FROM_UNIXTIME(time / 1000)) = CURDATE()",
    [REWARD_TYPES_MAP.ATTENDANCE_BONUS, userInfo.phone],
  );
  const [[adminAc]] = await connection.execute(
    `SELECT financial_setting FROM admin_ac LIMIT 1`,
  );
  console.log({ adminAc });
  let parsed = JSON.parse(adminAc.financial_setting);
  ...
  const timeLeft = diffInSeconds > 0 ? diffInSeconds : 0;
  if (timeLeft === 0) {
    return res.status(200).json({
      message: "You can not claim attendance bonus right now.",
      status: false,
      timeStamp: timeNow,
    });
  }
  const income =
    (Number(userInfo.money) * Number(parsed.dailyAttendanceIncome)) / 100;
  ...
  await connection.execute(
    ...
    [income, userInfo.phone],
  );

  return res.status(200).json({
    message: `Attendance Bonus ${income} has been claimed successfully.`,
    status: true,
    timeStamp: timeNow,
  });
};
...
const distributeSalaries = async () => {
  try {
    ...
    let offset = 0;
    const limit = 20; // Process 20 users at a time
      ...
      await Promise.all(
        users.map(async (user) => {
          const [members] = await connection.execute(
            "SELECT phone FROM users WHERE invite = ? AND is_active = 1",
            [user.code],
          );

          const activeMembersCount = members.length;
          if (activeMembersCount === 0) return;

          let salary = 0;
          if (activeMembersCount >= 5 && activeMembersCount < 10) {
            salary = salaryMap[5];
          } else if (activeMembersCount >= 10 && activeMembersCount < 20) {
            salary = salaryMap[10];
          } else if (activeMembersCount >= 20 && activeMembersCount < 40) {
            salary = salaryMap[20];
          } else if (activeMembersCount >= 40 && activeMembersCount < 60) {
            salary = salaryMap[40];
          } else if (activeMembersCount >= 60 && activeMembersCount < 80) {
            salary = salaryMap[60];
          } else if (activeMembersCount >= 80 && activeMembersCount < 100) {
            salary = salaryMap[80];
          } else if (activeMembersCount >= 100 && activeMembersCount < 125) {
            salary = salaryMap[100];
          } else if (activeMembersCount >= 125 && activeMembersCount < 150) {
            salary = salaryMap[125];
          } else if (activeMembersCount >= 150 && activeMembersCount < 200) {
            salary = salaryMap[150];
          } else if (activeMembersCount >= 200) {
            salary = salaryMap[200];
          }

          if (salary > 0 && user.is_active == 1) {
            const userPhones = [
              user.phone,
            ...
            const [[betUsers]] = await connection.execute(
              `SELECT SUM(money) as totalAmount FROM recharge WHERE phone IN (${userPhones}) AND DATE(FROM_UNIXTIME(time / 1000)) = CURDATE()`,
            );

            const totalBetAmount = betUsers?.totalAmount || 0;

            if (totalBetAmount >= 50000) {
              const reward_id = generateClaimRewardID();
    ...
    console.log("Error from distributeSalaries:", err);
  }
};

const distributeTeamBonuses = async () => {
  try {
    const teamBonusMap = {
      3: { deposit: 555, bonus: 199 },
      7: { deposit: 666, bonus: 299 },
      10: { deposit: 999, bonus: 699 },
      25: { deposit: 999, bonus: 1899 },
      50: { deposit: 1333, bonus: 2299 },
      70: { deposit: 1333, bonus: 3399 },
      100: { deposit: 1666, bonus: 4999 },
      200: { deposit: 1666, bonus: 6999 },
      500: { deposit: 1888, bonus: 15999 },
      1000: { deposit: 1888, bonus: 15999 },
      2000: { deposit: 2111, bonus: 47999 },
      5000: { deposit: 2333, bonus: 299999 },
      10000: { deposit: 2333, bonus: 9999999 },
    };
    ...
    while (true) {
      ...
      await Promise.all(
        users.map(async (user) => {
          const [activeMembersCountQuery] = await connection.execute(
            "SELECT COUNT(*) as activeMembersCount FROM users WHERE invite = ?",
            [user.code],
          );

          const activeMembersCount =
            activeMembersCountQuery[0].activeMembersCount || 0;

          if (activeMembersCount > 0) {
            let teamBonusObj;
            if (activeMembersCount >= 3 && activeMembersCount < 7) {
              teamBonusObj = teamBonusMap[3];
            } else if (activeMembersCount >= 7 && activeMembersCount < 10) {
              teamBonusObj = teamBonusMap[7];
            } else if (activeMembersCount >= 10 && activeMembersCount < 25) {
              teamBonusObj = teamBonusMap[10];
            } else if (activeMembersCount >= 25 && activeMembersCount < 50) {
              teamBonusObj = teamBonusMap[25];
            } else if (activeMembersCount >= 50 && activeMembersCount < 70) {
              teamBonusObj = teamBonusMap[50];
            } else if (activeMembersCount >= 70 && activeMembersCount < 100) {
              teamBonusObj = teamBonusMap[70];
            } else if (activeMembersCount >= 100 && activeMembersCount < 200) {
              teamBonusObj = teamBonusMap[100];
            } else if (activeMembersCount >= 200 && activeMembersCount < 500) {
              teamBonusObj = teamBonusMap[200];
            } else if (activeMembersCount >= 500 && activeMembersCount < 1000) {
              teamBonusObj = teamBonusMap[500];
            } else if (
              activeMembersCount >= 1000 &&
              activeMembersCount < 2000
            ) {
              teamBonusObj = teamBonusMap[1000];
            } else if (
              activeMembersCount >= 2000 &&
              activeMembersCount < 5000
            ) {
              teamBonusObj = teamBonusMap[2000];
            } else if (
              activeMembersCount >= 5000 &&
              activeMembersCount < 10000
            ) {
              teamBonusObj = teamBonusMap[5000];
            } else if (activeMembersCount > 10000) {
              teamBonusObj = teamBonusMap[10000];
            }

            if (teamBonusObj) {
              const [mems] = await connection.execute(
                "SELECT phone FROM users WHERE invite = ?",
                [user.code],
              );

              if (mems.length > 0) {
                const userPhones = mems.map((mem) => mem.phone).join(",");

                const [deposits] = await connection.execute(
                  `SELECT phone, SUM(money) as total_amount FROM recharge WHERE phone IN (${userPhones}) AND status = 1 GROUP BY phone`,
                );

                let bonusEligible = deposits.length > 0;

                for (let deposit of deposits) {
                  if ((deposit.total_amount || 0) < teamBonusObj.deposit) {
                    bonusEligible = false;
                    break;
                  }
                }
                ...
                if (bonusEligible) {
                  ...
                  await connection.execute(
                    ...
                    [
                      user.phone,
                      reward_id,
                      REWARD_TYPES_MAP.DAILY_TEAM_BONUS,
                      teamBonusObj.bonus,
                      1,
                      currentTime,
                    ],
    ...
    console.log("Error from distributeTeamBonuses:", err);
  }
};

const distributeDailyInterest = async () => {
  try {
    let offset = 0;
    const limit = 20; // Process 20 users at a time
    ...
    while (true) {
      ...
      await Promise.all(
        users.map(async (user) => {
          const reward_id = generateClaimRewardID();
          if (parseFloat(user.money) > 0) {
            let bonus = parseFloat(
              (parseFloat(user.money) *
                parseFloat(process.env.DAILY_INTEREST)) /
                100,
            );
            ...
            if (bonus >= 1) {
              await connection.execute(
                ...
                [
                  user.phone,
                  reward_id,
                  REWARD_TYPES_MAP.DAILY_INTEREST,
                  bonus,
                  1,
                  Date.now(),
                ],
    ...
    console.log("Error from distributeDailyInterest:", err);
  }
};

const distributeExtraSalaryDaily = async () => {
  try {
    const extraSalaryList = [
      { minimumTotalRechargeForTheDay: 1000000, salary: 30000 },
      { minimumTotalRechargeForTheDay: 500000, salary: 15000 },
      { minimumTotalRechargeForTheDay: 300000, salary: 12000 },
      { minimumTotalRechargeForTheDay: 200000, salary: 10000 },
      { minimumTotalRechargeForTheDay: 150000, salary: 6000 },
      { minimumTotalRechargeForTheDay: 100000, salary: 5000 },
      { minimumTotalRechargeForTheDay: 75000, salary: 3000 },
      { minimumTotalRechargeForTheDay: 50000, salary: 2000 },
    ];
    ...
    while (true) {
      ...
      await Promise.all(
        users.map(async (user) => {
          const [totalRechargeAmountForToday] = await connection.execute(
            "SELECT SUM(money) as total_amount FROM recharge WHERE phone = ? AND status = 1 AND DATE(FROM_UNIXTIME(time / 1000)) = CURDATE()",
            [user.phone],
          );

          const [totalRefferals] = await connection.execute(
            "SELECT COUNT(*) as total_count FROM users WHERE invite = ? AND DATE(FROM_UNIXTIME(time / 1000)) = CURDATE()",
            [user.code],
          );

          const refUsersCount = totalRefferals[0].total_count || 0;
          const totalAmountOfRechargeOfToday =
            totalRechargeAmountForToday[0].total_amount || 0;

          // check if today referral total is 3 or greater and their recharge amount is greater than 0
          if (refUsersCount >= 3 && totalAmountOfRechargeOfToday > 0) {
            const elilibleSalaryObject = extraSalaryList.find(
              (item) =>
                totalAmountOfRechargeOfToday >=
                item.minimumTotalRechargeForTheDay,
            );

            if (elilibleSalaryObject) {
              const reward_id = generateClaimRewardID();
              const bonus = parseFloat(elilibleSalaryObject.salary);
              await connection.execute(
                "INSERT INTO claimed_rewards SET phone = ?, reward_id = ?, type = ?, amount = ?, status = ? , time = ?",
                [
                  user.phone,
                  reward_id,
                  REWARD_TYPES_MAP.EXTRA_SALARY,
                  bonus,
                  1,
                  timeNow,
                ],
    ...
    console.log("error from distributeExtraSalaryDaily:", err);
  }
};

const distributeMonthlyLevelIncome = async () => {
  try {
    const [[setting]] = await connection.query(
      "SELECT referral_level_income_setting FROM admin_ac WHERE id = 1",
    );

    const incomesLevels = JSON.parse(setting.referral_level_income_setting);
    const batchSize = 20;
    let offset = 0;
    ...
    while (true) {
      ...
      if (users.length === 0) {
        // No more users left to process, exit the loop
        break;
      }
      ...
      await Promise.all(
        users.map(async (userInfo) => {
          const [totalRecharge] = await connection.execute(
            `SELECT SUM(money) as totalAmount FROM recharge 
           WHERE phone = ? 
           AND status = 2 
           AND DATE(FROM_UNIXTIME(time / 1000)) 
           BETWEEN DATE_SUB(CURDATE(), INTERVAL 1 MONTH) - INTERVAL DAY(CURDATE())-1 DAY  
           AND LAST_DAY(DATE_SUB(CURDATE(), INTERVAL 1 MONTH))`,
            [userInfo.phone],
          );

          if ((totalRecharge[0]?.totalAmount || 0) > 0) {
            const [parentMembers] = await connection.execute(
              `
             WITH RECURSIVE ParentCTE AS (
            SELECT 
                u1.id AS user_id, 
                u1.name_user AS name_user,
                u1.phone AS phone,
                u1.invite AS parent_code, 
            ...
            SELECT 
                u2.id AS user_id, 
                u2.name_user AS name_user,
                u2.phone AS phone,
                u2.invite AS parent_code, 
                u2.code AS user_code, 
                p.level + 1 AS level
            FROM 
                users u2
            INNER JOIN 
                ParentCTE p ON u2.code = p.parent_code
            WHERE 
                p.level < 10
        ),
        RankedCTE AS (
            SELECT 
                *, 
                ROW_NUMBER() OVER (PARTITION BY phone ORDER BY level ASC) AS row_num
            FROM 
                ParentCTE
        )
        SELECT 
        ...
        WHERE user_code != ? AND row_num = 1
            ...
            if (parentMembers.length > 0) {
              await Promise.all(
                parentMembers.map(async (parentMember) => {
                  const userActualLevel = parentMember.level - 1;
                  if (incomesLevels.hasOwnProperty(`level${userActualLevel}`)) {
                    const commissionAmount =
                      (parseFloat(totalRecharge[0]?.totalAmount || 0) *
                        parseFloat(incomesLevels[`level${userActualLevel}`])) /
                      100;
    ...
    console.error("Error from distributeMonthlyLevelIncome:", err);
...
const userController = {
  ...
  updateRecharge,
  confirmRecharge,
  claimDailyAttendanceBonus,
  cancelRecharge,
  confirmUSDTRecharge,
  distributeSalaries,
  distributeTeamBonuses,
  distributeDailyInterest,
  distributeExtraSalaryDaily,
  distributeMonthlyLevelIncome,
};
