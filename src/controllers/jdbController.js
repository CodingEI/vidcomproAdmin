...
const generateGameLink = async (req, res) => {
    ...
    // Fetch iv, key, parent, and dc from environment variables
    const iv = process.env.JDB_IV;
    const key = process.env.JDB_KEY;
    const parent = process.env.JDB_AGENT_ID;
    const dc = process.env.JDB_DC; // Fetch dc from the environment

    try {
        // Check for required parameters
        if (!token || !iv || !key || !parent || !dc) {
        ...
        // Define the API URL using the dc from the environment
        const apiUrl = `${process.env.JDB_API_URL}/apiRequest.do?dc=${dc}&x=${encodedData}`;
