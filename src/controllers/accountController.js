...
const register = async (req, res) => {
  try {
    const schema = Joi.object({
      phoneNumber: Joi.string().length(10).required(),
      pwd: Joi.string().min(6).required(),
      invitecode: Joi.string().allow("").required(),
      dialCode: Joi.string().required(),
    });
    ...
    let [setting] = await connection.execute("SELECT * FROM admin_ac LIMIT 1");
    // let bonus_money = setting[0].signup_bonus;
    let bonus_money = 0;
