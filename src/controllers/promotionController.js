...
const subordinatesDataAPI = async (req, res) => {
  try {
    ...
    const [directSubords] = await connection.execute(
    ...
    const [allSubordinates] = await connection.execute(
      `WITH RECURSIVE team AS (
          ...
          SELECT phone, id, code, invite, time
          FROM users
          WHERE code = ?
          
          UNION ALL
         
          SELECT u.phone, u.id, u.code, u.invite, u.time
          FROM users u
          INNER JOIN team t ON u.invite = t.code
      )
      SELECT * FROM team WHERE invite != ? AND phone != ? AND DATE(FROM_UNIXTIME(time / 1000)) = CURDATE() `,
      [user.code, user.code, user.phone],
    );

    const directSubOrdsPhones = directSubords.map((it) => it.phone);

    let mdirectSubOrdsDepositNum = 0;
    let mdirectSubOrdsTotalDepositAmount = 0;
    let mdirectSubOrdsWithFirstDeposit = 0;

    if (directSubOrdsPhones.length > 0) {
      const [directSubOrdsDepositNum] = await connection.execute(
        `SELECT COUNT(*) as count FROM recharge WHERE status = 1 AND phone IN (${directSubOrdsPhones.map((e) => "?").join(",")}) AND DATE(FROM_UNIXTIME(time / 1000)) = CURDATE()`,
        directSubOrdsPhones,
      );
      if (
        directSubOrdsDepositNum.length > 0 &&
        directSubOrdsDepositNum[0].count
      ) {
        mdirectSubOrdsDepositNum = directSubOrdsDepositNum[0].count || 0;
      }

      const [directSubOrdsTotalDepositAmount] = await connection.execute(
        `SELECT SUM(money) as amount FROM recharge WHERE status = 1 AND phone IN (${directSubOrdsPhones.map((e) => "?").join(",")}) AND DATE(FROM_UNIXTIME(time / 1000)) = CURDATE()`,
        directSubOrdsPhones,
      );

      if (
        directSubOrdsTotalDepositAmount.length > 0 &&
        directSubOrdsTotalDepositAmount[0].amount
      ) {
        mdirectSubOrdsTotalDepositAmount =
          directSubOrdsTotalDepositAmount[0].amount || 0;
      }

      const [directSubOrdsWithFirstDeposit] = await connection.execute(
        `SELECT COUNT(*)  as count FROM recharge WHERE status = 1 AND phone IN (${directSubOrdsPhones.map((e) => "?").join(",")}) AND DATE(FROM_UNIXTIME(time / 1000)) = CURDATE() GROUP BY phone  HAVING COUNT(*) = 1`,
        directSubOrdsPhones,
      );
      if (directSubOrdsWithFirstDeposit.length > 0) {
        mdirectSubOrdsWithFirstDeposit = directSubOrdsWithFirstDeposit.length;
      }
    }

    // team subordinates
    const teamSubOrdsPhones = allSubordinates.map((it) => it.phone);

    let mteamSubOrdsDepositNum = 0;
    let mteamSubOrdsTotalDepositAmount = 0;
    let mteamSubOrdsWithFirstDeposit = 0;

    if (teamSubOrdsPhones.length > 0) {
      const [teamSubOrdsDepositNum] = await connection.execute(
        `SELECT COUNT(*) as count FROM recharge WHERE status = 1 AND phone IN (${teamSubOrdsPhones.map((e) => "?").join(",")}) AND DATE(FROM_UNIXTIME(time / 1000)) = CURDATE()`,
        teamSubOrdsPhones,
      );
      if (teamSubOrdsDepositNum.length > 0 && teamSubOrdsDepositNum[0].count) {
        mteamSubOrdsDepositNum = teamSubOrdsDepositNum[0].count || 0;
      }

      const [teamSubOrdsTotalDepositAmount] = await connection.execute(
        `SELECT SUM(money) as amount FROM recharge WHERE status = 1 AND phone IN (${teamSubOrdsPhones.map((e) => "?").join(",")}) AND DATE(FROM_UNIXTIME(time / 1000)) = CURDATE()`,
        teamSubOrdsPhones,
      );

      if (
        teamSubOrdsTotalDepositAmount.length > 0 &&
        teamSubOrdsTotalDepositAmount[0].amount
      ) {
        mteamSubOrdsTotalDepositAmount =
          teamSubOrdsTotalDepositAmount[0].amount || 0;
      }

      const [teamSubOrdsWithFirstDeposit] = await connection.execute(
        `SELECT COUNT(*)  as count FROM recharge WHERE status = 1 AND phone IN (${teamSubOrdsPhones.map((e) => "?").join(",")}) AND DATE(FROM_UNIXTIME(time / 1000)) = CURDATE() GROUP BY phone  HAVING COUNT(*) = 1`,
        teamSubOrdsPhones,
      );
      if (
        teamSubOrdsWithFirstDeposit.length > 0 &&
        teamSubOrdsWithFirstDeposit[0].count
      ) {
        mteamSubOrdsWithFirstDeposit =
          teamSubOrdsWithFirstDeposit[0].count || 0;
      }
    }

    const [totalComissionRows] = await connection.execute(
      `SELECT SUM(amount) as total
      FROM claimed_rewards WHERE  phone = ?`,
      [user.phone],
    );

    const [thisWeekCommissionRows] = await connection.execute(
      `SELECT SUM(amount) as total
      FROM claimed_rewards WHERE  phone = ? AND time >= UNIX_TIMESTAMP(DATE_SUB(CURDATE(), INTERVAL WEEKDAY(CURDATE()) DAY))
      AND time < UNIX_TIMESTAMP(DATE_ADD(DATE_SUB(CURDATE(), INTERVAL WEEKDAY(CURDATE()) DAY), INTERVAL 7 DAY));`,
      [user.phone],
    );
    const [yesterdayComissionRows] = await connection.execute(
      `SELECT SUM(amount) as total
            FROM claimed_rewards
            WHERE phone = ? AND time >= UNIX_TIMESTAMP(DATE_SUB(CURDATE(), INTERVAL 1 DAY))
            AND time < UNIX_TIMESTAMP(CURDATE());`,
      [user.phone],
    );

    const [todayDirectSubordinates] = await connection.execute(
      `SELECT COUNT(*) as count
            FROM users
            WHERE invite = ? AND DATE(FROM_UNIXTIME(time / 1000)) = CURDATE()`,
      [user.code],
    );

    const [todayTeamBonus] = await connection.execute(
      `SELECT SUM(amount) as count
            FROM claimed_rewards
            WHERE phone = ? AND type = ? AND DATE(FROM_UNIXTIME(time / 1000)) = CURDATE()`,
      [user.phone, REWARD_TYPES_MAP.DAILY_TEAM_BONUS],
    );

    let thisWeekCommissionAmount = 0;
    let yesterdayCommissionAmount = 0;
    let totalCommisionAm = 0;
    if (thisWeekCommissionRows.length > 0 && thisWeekCommissionRows[0].total) {
      thisWeekCommissionAmount = thisWeekCommissionRows[0].total;
    }
    if (totalComissionRows.length > 0 && totalComissionRows[0].total) {
      totalCommisionAm = totalComissionRows[0].total;
    }
    if (yesterdayComissionRows.length > 0 && yesterdayComissionRows[0].total) {
      yesterdayCommissionAmount = yesterdayComissionRows[0].total;
    }

    const { usersByLevels = [], level1Referrals = [] } = {};
    ...
    const [directSubordinateCountQuery] = await connection.execute(
      "SELECT COUNT(*) as count FROM users WHERE invite = ?",
      [user.code],
    );

    const [allSubordinatesCountsQuery] = await connection.execute(
      `WITH RECURSIVE team AS (
      ...
      SELECT COUNT(*) as count FROM team WHERE invite != ? AND phone != ?`,
      [user.code, user.code, user.phone],
    );

    const [[setting]] = await connection.query(
      "SELECT * FROM admin_ac WHERE id = 1",
    );

    const [totalTodayWingoBetAmount] = await connection.execute(
      `SELECT SUM(bet) as total
            FROM minutes_1
            WHERE phone = ? AND DATE(FROM_UNIXTIME(time / 1000)) = CURDATE() AND status != 0`,
      [user.phone],
    );

    const betAmountBasedOnProvidedPercentage =
      (totalTodayWingoBetAmount[0]?.total *
        parseFloat(
          JSON.parse(setting.financial_setting).dailyTeamIncomePercentage,
        )) /
      100;

    return res.status(200).json({
      data: {
        directSubordinatesCount: directSubordinateCountQuery[0]?.count || 0,
        noOfRegisteredSubordinates: directSubords.length,
        directSubordinatesRechargeQuantity: mdirectSubOrdsDepositNum,
        directSubordinatesRechargeAmount: mdirectSubOrdsTotalDepositAmount,
        directSubordinatesWithDepositCount: mdirectSubOrdsWithFirstDeposit,
        teamSubordinatesCount: allSubordinatesCountsQuery[0]?.count || 0,
        noOfRegisterAllSubordinates: teamSubOrdsPhones.length || 0,
        teamSubordinatesRechargeQuantity: mteamSubOrdsDepositNum,
        teamSubordinatesRechargeAmount: mteamSubOrdsTotalDepositAmount,
        teamSubordinatesWithDepositCount: mteamSubOrdsWithFirstDeposit,
        totalCommissions: totalCommisionAm,
        totalCommissionsThisWeek: thisWeekCommissionAmount,
        totalCommissionsYesterday: yesterdayCommissionAmount,
        todayDirectSubordinates: todayDirectSubordinates[0]?.count || 0,
        todayTeamBonus: todayTeamBonus[0]?.count || 0,
        todayBetAmount: betAmountBasedOnProvidedPercentage,
      },
    });
  } catch (error) {
    console.log(error);
...
const subordinatesDataByTimeAPI = async (req, res) => {
  try {
    ...
    const sortedUsersByBet = filteredUsers.sort(
      (a, b) => b.total_bet_amount - a.total_bet_amount,
    );
    ...
    const paginatedUsers = sortedUsersByBet;
    ...
    res.json({
      ...
      data: {
        ...
        subordinatesRechargeAmount,
        subordinatesWithBettingCount,
        subordinatesBettingAmount: betAmountBasedOnProvidedPercentage,
...
export const InvitationBonusList = [
  {
    id: 1,
  ...
  {
    id: 2,
    numberOfInvitedMembers: 7,
    numberOfDeposits: 0,
    amountOfRechargePerPerson: 666,
    bonusAmount: 299,
  },
  {
    id: 3,
    ...
    numberOfDeposits: 0,
    amountOfRechargePerPerson: 999,
    bonusAmount: 699,
  },
  {
    id: 4,
    numberOfInvitedMembers: 25,
    numberOfDeposits: 0,
    amountOfRechargePerPerson: 999,
    bonusAmount: 1899,
  },
  {
    ...
    numberOfDeposits: 0,
    amountOfRechargePerPerson: 1333,
    bonusAmount: 2299,
  },
  {
    id: 6,
  ...
  },
  {
    ...
    numberOfDeposits: 0,
    amountOfRechargePerPerson: 1666,
    bonusAmount: 4999,
  },
  {
    id: 8,
  ...
  },
  {
    ...
    numberOfDeposits: 0,
    amountOfRechargePerPerson: 1888,
    bonusAmount: 15999,
  },
  {
    id: 10,
  ...
  },
  {
    ...
    numberOfDeposits: 0,
    amountOfRechargePerPerson: 2333,
    bonusAmount: 299999,
  },
  {
    id: 13,
